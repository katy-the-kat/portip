...
add_port() {
    ...
    ssh -o StrictHostKeyChecking=no -f -N -R ${random_port}:localhost:${local_port} 138.68.79.95 > /dev/null &
    ssh_pid=$!
...
remove_port() {
    ...
    pkill -f "ssh -o StrictHostKeyChecking=no -f -N -R ${random_port}:localhost:${local_port} 138.68.79.95" > /dev/null
