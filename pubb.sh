...
add_port() {
    ...
    ssh -o StrictHostKeyChecking=no -f -N -R 0.0.0.0:${random_port}:localhost:${local_port} u@178.16.140.30 > /dev/null &
    ssh_pid=$!
...
remove_port() {
    ...
    pkill -f "ssh -o StrictHostKeyChecking=no -f -N -R 0.0.0.0:${random_port}:localhost:${local_port} u@178.16.140.30" > /dev/null
